...
export const projectsApi = {
  create: (payload: ProjectCreatePayload): Promise<Project> =>
    httpRequest<Project>('/projects', { method: 'POST', body: payload }),
  list: (query: ProjectsQueryParams = {}): Promise<PaginatedProjects> =>
    httpRequest<PaginatedProjects>('/projects', { query: query as Record<string, string | number | boolean | string[] | undefined | null> }),
  getById: (projectId: string): Promise<Project> =>
    httpRequest<Project>(`/projects/${projectId}`),
  ...
  delete: (projectId: string): Promise<null> =>
    httpRequest<null>(`/projects/${projectId}`, { method: 'DELETE' }),
};
