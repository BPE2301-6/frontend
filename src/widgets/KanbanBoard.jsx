...
function TaskCard({ title, description, date, type, priority, assignee, className = "" }) {
  const getPriorityColor = () => {
    switch (priority) {
      case 'high': return 'bg-figma-red';
      case 'medium': return 'bg-figma-yellow';
      case 'low': return 'bg-figma-green';
      default: return 'bg-figma-red';
    }
  };

  return (
    <div className={`w-figma-card-width h-figma-card-height bg-figma-card rounded-figma-card relative ${className}`}>
      {/* Приоритет */}
      <div className={`absolute w-[17px] h-[17px] rounded-full ${getPriorityColor()} top-[18px] right-[24px]`}></div>
...
function KanbanColumn({ title, tasks, className = "" }) {
  return (
    <div className={`w-figma-column-width h-figma-column-height border border-figma-blue border-t-0 rounded-t-figma-column relative ${className}`}>
      {/* Заголовок колонки */}
      ...
      <div className="absolute w-full h-figma-header top-0 left-0 bg-figma-bg border-b border-figma-blue">
        ...
        <div className="absolute left-[360px] top-[19px] flex space-x-[8px]">
          {['A', 'B', 'C'].map((user, index) => (
            <div key={index} className="w-9 h-9 bg-figma-white rounded-full flex items-center justify-center">
              <div className="w-6 h-6 border-4 border-figma-user-border rounded-full"></div>
            </div>
          ))}
        </div>
